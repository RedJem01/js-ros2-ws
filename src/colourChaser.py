...
class ColourChaser(Node):
    def __init__(self):
        ...
        self.scan = None
        self.movingToObject = None
        self.movedLastCall = None
        self.pushingObject = False

        # publish cmd_vel topic to move the robot
        self.pub_cmd_vel = self.create_publisher(Twist, 'cmd_vel', 1)
        
        self.create_subscription(Odometry, '/odom', self.set_pos, 1)
    ...
    def camera_callback(self, data):
        ...
        if (self.movingToObject == True):
            self.movedLastCall = True
        else:
            self.movedLastCall = False
        
        print("pushingObject = " + str(self.pushingObject))
        if (self.pushingObject == True):
            self.tw.angular.z = 0.0
            self.tw.linear.x = 1.0
            self.movedLastCall = True
            print(self.scan.range_max)
            if (self.scan.range_max < 2.0):
                self.pushingObject = False
                self.movingToObject = False
        else:
            if len(contours) > 0:
                ...
                if M['m00'] > 0:
                    ...
                    # else if center of object is to the right of image center move right
                        ...
                        self.tw.angular.z=0.0
                        self.tw.linear.x=1.0
                    self.movingToObject = True
                        
            else:
                print("No Centroid Found")
                # turn until we can see a coloured object
                self.tw.angular.z=0.3
                if (self.pushingObject == False):
                    self.movingToObject = False

        self.pub_cmd_vel.publish(self.tw)
        print(self.movedLastCall)
        print(len(contours))
        if (len(contours) == 0 and self.movedLastCall == True):
            self.movingToObject = True
    ...
    def set_pos(self, msg):
        ...
        self.set_robot_new_pose = np.array([msg.pose.pose.position.x, msg.pose.pose.position.y, yaw])
